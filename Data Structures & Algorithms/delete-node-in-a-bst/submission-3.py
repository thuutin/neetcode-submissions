    ...
    def deleteNode(self, root: Optional[TreeNode], key: int) -> Optional[TreeNode]:
        ...
        if root.val == key:
            if root.right:
                ...
                right = self.deleteNode(root.right, minFromRight)
                return TreeNode(minFromRight, root.left, right) 
            elif root.left:
                maxFromLeft = self.minMax[root.left][1]                
                left = self.deleteNode(root.left, maxFromLeft)
                return TreeNode(maxFromLeft, left, root.right)
            else:
                return None
        elif root.val < key:
